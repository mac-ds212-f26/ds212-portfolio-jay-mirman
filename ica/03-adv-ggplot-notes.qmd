...
- Relate the intro paragraph: "Scientists declared that 2015 was Earth’s hottest year on record..." to the *design* of the visualization.  In particular, based on the intro paragraph,
  ...
  - How did this goal inform what information is displayed in the visualization?
    ...
    It informed them to include data specific to showing how 2015 was hot in comparison to previous years

**Aesthetic Mapping** 

- What specific variables (from the data codebook) underlie the visualization?

    NormalLow/NormalHigh/RecordLow/RecordHigh/Low/High are the most important, but the vis also uses date, precepitation, Record, and others

- How do these variables map to aesthetics of the visual elements, eg, position, size, shape, and color of glyphs?

    High/Low are the most easily seen visual elements, while the map uses Record to point out specific dates that reached a new record
...
ggplot(weather) +
  ...
  geom_vline(
    xintercept = c(31,59,90,121,150,181,211,242,272,303,333,365),
    slope = 100000,
    color = "gray"
  ) +
  theme_classic()
...
weather_chart <- weather_chart + labs(x = "Month")

weather_chart + 
  scale_x_continuous(
    breaks = c(31,59,90,121,150,181,211,242,272,303,333,365),
    labels = month.abb
  )
...
precip_chart <- ggplot(weather, aes(x = dateInYear, y = CulmPrec)) +
  geom_area(color = "#32a3d8", fill = "#ebeae2") +
  theme_classic()
